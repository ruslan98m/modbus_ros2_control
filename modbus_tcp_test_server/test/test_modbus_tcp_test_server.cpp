...
TEST(ModbusTcpTestServer, WriteAndReadHoldingRegister) {
  ...
  EXPECT_EQ(modbus_write_register(ctx, reg_addr, write_val), 1) << modbus_strerror(errno);

  uint16_t read_val = 0;
  EXPECT_EQ(modbus_read_registers(ctx, reg_addr, 1, &read_val), 1) << modbus_strerror(errno);
...
TEST(ModbusTcpTestServer, WriteAndReadMultipleHoldingRegisters) {
  ...
  EXPECT_EQ(modbus_write_registers(ctx, start_addr, 4, write_tab), 4) << modbus_strerror(errno);

  uint16_t read_tab[4] = {0, 0, 0, 0};
  EXPECT_EQ(modbus_read_registers(ctx, start_addr, 4, read_tab), 4) << modbus_strerror(errno);
...
TEST(ModbusTcpTestServer, ReadInputRegisters) {
  ...
  EXPECT_EQ(modbus_read_input_registers(ctx, 0, 2, read_tab), 2) << modbus_strerror(errno);
