...
def _get_build_robot_description():
    ...
    launch_path = os.path.join(
        pkg_share, "launch", "test_modbus_hw_with_server.launch.py"
    )
...
class TestRobotDescription(unittest.TestCase):
    ...
    def test_urdf_ros2_control_and_plugin(self):
        ...
        if plugin_el is None:
            plugin_el = next(
                (p for p in hardware.findall("param") if p.get("name") == "plugin"),
                None,
            )
    ...
    def test_urdf_modbus_tcp_params(self):
        ...
        self.assertIsNotNone(hardware)
        params = {
            p.get("name"): (p.text or p.get("value") or "").strip()
            for p in hardware.findall("param")
        }
...
class TestModbusLifecycle(unittest.TestCase):
    ...
    def test_hardware_component_loaded_and_active(self):
        ...
        try:
            ...
            if not client.wait_for_service(timeout_sec=10.0):
                self.skipTest(
                    "controller_manager/list_hardware_components not available"
                )
            ...
            self.assertIsNotNone(
                modbus, f"ModbusTCP not in components: {[c.name for c in components]}"
            )
            self.assertEqual(
                modbus.state.id,
    ...
    def test_hardware_interfaces_exported(self):
        ...
        try:
            ...
            if not client.wait_for_service(timeout_sec=10.0):
                self.skipTest(
                    "controller_manager/list_hardware_interfaces not available"
                )
            ...
            self.assertIn(
                "ModbusTCP/tcp_bus_plc_1_temperature",
                state_names,
                f"Expected temperature state interface in {state_names}",
            )
            self.assertIn(
                "ModbusTCP/tcp_bus_plc_1_setpoint",
                cmd_names,
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.skipTest(
                        "ModbusTCP/plc_1 not in dynamic_joint_states (joint_names: %s)"
            ...
            self.assertIsNotNone(
                temp_key,
                "temperature interface not found in %s" % list(name_to_val.keys()),
            )
            self.assertAlmostEqual(
                float(name_to_val[temp_key]),
                0.0,
                places=5,
            ...
            self.assertIsNotNone(
                counter_key,
                "counter interface not found in %s" % list(name_to_val.keys()),
            )
            self.assertAlmostEqual(
                float(name_to_val[counter_key]),
                counter_expected,
                places=0,
